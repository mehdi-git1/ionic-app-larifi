...
@Component({
    selector: 'synchro-request-type',
    templateUrl: 'synchro-request-type.page.html',
    styleUrls: ['./synchro-request-type.page.scss']
})
export class SynchroRequestTypeComponent {

    @Input() synchronizationType: SynchroRequestTypeEnum;
        ...
        if (this.synchroRequestList) {
            if (synchroRequestDisplayModeEnum === SynchroRequestDisplayModeEnum.PENDING) {
                return this.synchroRequestList.filter(synchroRequest => {
                    return this.synchronizationType === synchroRequest.requestType
