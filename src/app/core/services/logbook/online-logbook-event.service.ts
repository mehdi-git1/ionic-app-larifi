...
import { LogbookEventFilterModel } from '../../models/logbook/logbook-event-filter.model';
import { LogbookEventModel } from '../../models/logbook/logbook-event.model';
import { PagedGenericModel } from '../../models/paged-generic-model';
...
export class OnlineLogbookEventService {
  ...
  /**
   * Récupère tous les évènements du journal de bord d'un pnc
  ...
  /**
   * Récupère tous évènements du journal de bord d'un pnc
   * @param matricule le matricule du pnc dont on souhaite récupérer les évènements
  ...
  getLogbookEventsByFilters(matricule: string, filter: LogbookEventFilterModel): Promise<PagedGenericModel<LogbookEventModel>> {
    return this.restService.get(this.config.getBackEndUrl('getLogbookEventsByFilters', [matricule]), filter);
  }
